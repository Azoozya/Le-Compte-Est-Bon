...
// unsigned long old_parse_input_json(facture* head,char* filename,float* target)
// {
//   FILE *fp;
// 	char buffer[4096];
...
// 	struct json_object *value_i;
//   struct json_object *id;
// 	struct json_object *id_i;
//   float* table_value;
//   unsigned long* table_id;
//   unsigned long elements;
//   size_t n_elements;
// 	size_t i;
// 	//On extrait le contenu du fichier et on le met dans un buffer;
// 	fp = fopen(filename,"r");
// 	fread(buffer, 4096, 1, fp);
// 	fclose(fp);
// 	//On tokenise chacune des entrées et les valeurs conenues.
...
//
//
// 	//On converti les tokens en variable type 'C'
// 	n_elements = json_object_array_length(value);
//   elements = (unsigned long) n_elements;
//   *target = json_object_get_double(price);
//
//   table_value = malloc(n_elements*sizeof(float));
//   table_id = malloc(n_elements*sizeof(unsigned long));
//   if ( table_id == NULL || table_value == NULL)
//     {
//       perror("Parsing du json immpossible, impossible de sauver les données\n");
//     }
//   else
//     {
//       for(i=0 ; i<n_elements ;i++)
//       {
//         value_i = json_object_array_get_idx(value, i);
//         table_value[i] = json_object_get_double(value_i);
//         id_i = json_object_array_get_idx(id, i);
//         table_id[i] = json_object_get_int(id_i);
//       }
//
//       //On génère la structure facture avec les valeurs données et on return n_elements
//       facture* stone_T;
//       facture* cursor;
//       for (unsigned long rank = 0 ; rank < elements ; rank++)
//         {
...
//       json_object_put(id);
//       json_object_put(value_i);
//       json_object_put(id_i);
